	...
	private class SelectThread extends Thread {
				...
				try {
					try {
						if (timeoutWatchSet.size() == 0) {
							...
							// We have a timeout pending, so calculate the time
							// until then and select appropriately
							long nextTimeout = timeoutWatchSet.first().getTimeoutTimestamp();
							long selectTime = nextTimeout - System.currentTimeMillis();
							if (selectTime > 0) {
								// Next timeout is in the future, select and
								// wake up then
								selector.select(selectTime);
							} else {
								// Next timeout is now or in past, select
								// immediately so we can time out
								selector.selectNow();
							}
						}
					} catch (IOException e) {
		...
		private void transitionMethods() {
			try {
				...
				while (keys.hasNext()) {
					...
					keys.remove();
					if (!key.isValid()) {
						// this can happen if the method call experienced an
						// error and the
						// key was cancelled. can also happen if we timeout a
						// method, which
		...
		private void startPendingMethods() {
			...
			while ((methodCall = pendingCalls.poll()) != null) {
				// Catch registration errors. method will catch transition
				// errors and cleanup.
				try {
					methodCall.start(selector);

					// If timeout specified and first transition went smoothly,
					// add to timeout watch set
