 ...
 * Consistent hash.
 *
 * <p> This implementation is concurrency safe; additions and removals are
 * serialized, but look up can be performed concurrently even when modifications
 * is in progress.
 *
 * <p> Since typical hash functions we use in {@link Object#hashCode()} isn't
 * random enough to evenly populate the 2^32 ring space, we only ask the user to
 * give us <a href="http://en.wikipedia.org/wiki/Injective_function">an
 * injective function</a> to a string, and then we use MD5 to create random
 * enough distribution.
 *
 * <p> This consistent hash implementaiton is consistent both to the
 * addition/removal of Ts, as well as increase/decrease of the replicas.
 *
 * <p> See http://en.wikipedia.org/wiki/Consistent_hashing for references, and
 * http://weblogs.java.net/blog/tomwhite/archive/2007/11/consistent_hash.html is
 * probably a reasonable depiction. If we trust his experiments, creating 100
 * replicas will reduce the stddev to 10% of the mean for 10 nodes.
 *
...
public class ConsistentHash<T> {
    ...
    /**
     * Used for remembering the computed MD5 hash, since it's bit expensive to
    ...
    private final class Table {
         ...
         *
         * <p> This is a permutation of all the nodes, where nodes with more
         * replicas are more likely to show up early on.
         */
     ...
     *
     * <p> By default, {@link ConsistentHash} uses {@link Object#toString()} on
     * 'T' to obtain the hash, but that behavior can be changed by providing a
     * {@link Hash} implementation.
     *
     * <p> This hash function need not produce a very uniform distribution, as
     * the output is rehashed with MD5. But it does need to make sure it doesn't
     * produce the same value for two different 'T's (and that's why this
     * returns String, not the usual int.)
     */
    public interface Hash<T> {

        /**
         * @param t The object to be hashed. Never null.
     ...
     *
     * <p> The whole point of this class is that if the same query point is
     * given, it's likely to return the same result even when other nodes are
     * added/removed, or the # of replicas for the given node is changed.
     *
     * @return null if the consistent hash is empty. Otherwise always non-null.
     */
     ...
     *
     * <p> The returned pemutation is consistent, in the sense that small change
     * to the consitent hash (like addition/removal/change of replicas) only
     * creates a small change in the permutation.
     *
     * <p> Nodes with more replicas are more likely to show up early in the list
     */
