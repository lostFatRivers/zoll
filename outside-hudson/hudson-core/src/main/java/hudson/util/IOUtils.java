...
public class IOUtils extends org.apache.commons.io.IOUtils {
    ...
    /**
     * Ensures that the given directory exists (if not, it's created, including
     * all the parent directories.)
     *
     * @return This method returns the 'dir' parameter so that the method call
     * flows better.
     */
     ...
     *
     * <p> {@link InputStream#skip(long)} has two problems. One is that it
     * doesn't let us reliably differentiate "hit EOF" case vs "inpustream just
     * returning 0 since there's no data currently available at hand", and some
     * subtypes (such as {@link FileInputStream#skip(long)} returning -1.
     *
     * <p> So to reliably skip just the N bytes, we'll actually read all those
     * bytes.
     *
     * @since 1.349
     * @deprecated use
     ...
     *
     * @param path String representing <code> Platform Specific </code> (unlike
     * FilePath, which may get Platform agnostic paths), may not be null
     * @return true if String represents absolute path on this platform, false
     * otherwise
     */
