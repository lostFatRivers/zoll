 ...
 *
 * <h2>Usage</h2> <p> The dynamic list box support allows the SELECT element to
 ...
 * </xmp></pre>
 *
 * <p> The SELECT element may have initial option values (in fact in most cases
 ...
 * too.)
 *
 * <p> Other parts of the HTML can initiate the SELECT element update by using
 * the "updateListBox" function, defined in <tt>hudson-behavior.js</tt>. The
 ...
 * </xmp></pre>
 *
 * <p> The first argument is the SELECT element or the ID of it (see
 * Prototype.js <tt>$(...)</tt> function.) The second argument is the URL that
 * returns the options list.
 *
 * <p> The URL usually maps to the <tt>doXXX</tt> method on the server, which
 ...
 * m.get(3).selected = true; return m; } </pre>
 * @since 1.123
...
public class ListBoxModel extends ArrayList<ListBoxModel.Option> implements HttpResponse {
    ...
    /**
     * A version of the {@link #add(String, String)} method where the display
     * name and the value are the same.
     */
