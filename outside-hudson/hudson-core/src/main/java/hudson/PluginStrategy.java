 ...
 *
 * <p> This extension point was added to allow plugins to be loaded into a
 * different environment (such as loading it in an existing DI container like
 * Plexus.) A plugin strategy is a singleton instance, and as such this feature
 * is primarily meant for OEM.
 *
 * See {@link PluginManager#createPluginStrategy()} for how this instance is
 * created.
 */
public interface PluginStrategy extends ExtensionPoint {

    /**
     * Creates a plugin wrapper, which provides a management interface for the
     * plugin
     *
     * @param archive Either a directory that points to a pre-exploded plugin,
     * or an hpi file, or an hpl file.
     */
     ...
     *
     * <p> This should be done after all the classloaders are constructed for
    ...
    /**
     * Optionally start services provided by the plugin. Should be called when
